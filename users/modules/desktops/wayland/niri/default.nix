{
  ...
  config = let
    ...
    useNiri = config.modules.desktops.wayland.enable
      && (config.modules.desktops.window_manager == "niri"
    ...
    programs.niri = {
      ...
      settings = {
        ...
        spawn-at-startup = [
          ...
          {argv = ["${pkgs.skwd-wall}/bin/skwd-daemon"];}
        ];
        prefer-no-csd = true;

        window-rules = [
          # inactive windows transparent, active windows opaque
          {
        ...
        binds = with config.lib.niri.actions; {
          ...
          # "Mod+Shift+Slash" = {
          #   hotkey-overlay.title = "Show Keyboard Shortcuts";
