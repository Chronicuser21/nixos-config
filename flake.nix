{
  ...
  inputs = {
    ...
    # tiny-dfr built from source
    tiny-dfr = {
      url = "github:AsahiLinux/tiny-dfr";
    ...
    # theming for boot splash and bootloader
  ...
  outputs = inputs: let
    ...
    # Global overlay to replace deno with pre-built binary for all packages
    overlay-deno = final: prev: {
      # Use pre-built deno for aarch64 to avoid compiling rusty-v8 from source
      deno = prev.callPackage /home/b/.local/deno.nix {};
    };
    ...
    overlay-extra = final: prev:
      ...
      in {
        ...
        skwd-wall = (inputs.skwd-wall.packages.${system}.default).overrideAttrs (old: {
          patches = (old.patches or []) ++ [ ./skwd-wall.patch ];
        });
        # Use pre-built deno for aarch64 to avoid compiling from source
        deno = prev.callPackage /home/b/.local/deno.nix {};
      };
    ...
    systems = {
      asahi = {
        platform = ./platforms/macbook-pro-15in-m1-2020/platform.nix;
        hardware-config = ./platforms/macbook-pro-15in-m1-2020/hardware-configuration.nix;
        host-config = ./hosts/asahi.nix;
        user-config = ./users/b-at-asahi.nix;
      };
    };
  in {
    ...
    nixosConfigurations = builtins.mapAttrs (system-name: cfg: 
      inputs.nixpkgs.lib.nixosSystem {
        ...
        modules = [
          ...
          # OpenClaw module
          ./hosts/modules/services/openclaw.nix
        ];
      }) systems;
